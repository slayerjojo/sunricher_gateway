...
static void AES128_ECB_encrypt(uint8_t* input, uint8_t *output)
{
    in = input;
    out = output;
    Cipher();
...
static void AES128_ECB_decrypt(uint8_t* input, uint8_t *output)
{
    in = input;
    out = output;
    InvCipher();
...
static void mix(unsigned char *buf, const unsigned char *mixData)
{
    uint8_t i;
    for(i = 0; i < 16; i++)
    {
...
long AES128CBCEncrypt(const unsigned char *in, unsigned char *out, long len, const unsigned char *key, const unsigned char *iv)
{
    ...
    while(cryptLen <= len)
    {
        ...
        {
            memcpy(cryptInBuf, in + cryptLen, 16);
        }

        mix(cryptInBuf, mixData);
        AES128_ECB_encrypt(cryptInBuf, cryptOutBuf);
        memcpy(out + cryptLen, cryptOutBuf, 16);
        memcpy(mixData, cryptOutBuf, 16);
        cryptLen += 16;
    }
    return cryptLen;
}

long AES128CBCDecrypt(const unsigned char *in, unsigned char *out, long len, const unsigned char *key, const unsigned char *iv)
{
    int cryptLen = 0;
    int pkcs5Num;
    uint8_t mixData[16];
    ...
    while(cryptLen < len)
    {
        memcpy(cryptInBuf, ((uint8_t *)in) + cryptLen, 16);
        AES128_ECB_decrypt(cryptInBuf, out + cryptLen);
        mix(out + cryptLen, mixData);
        memcpy(mixData, cryptInBuf, 16);
        cryptLen += 16;
    }
...
int AESEncrypt(const unsigned char *in, unsigned char *out, int len, const unsigned char *key, int keylen)
{
    int cryptLen = 0;
    int diffLen;
    uint8_t cryptInBuf[16];
...
int AESDecrypt(const unsigned char *in, unsigned char *out, int len, const unsigned char *key, int keylen)
{
    int cryptLen = 0;
    int pkcs5Num;
